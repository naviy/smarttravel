...
alter table lt_avia_document
drop foreign key aviadocument_commissiondiscount_currency_fk;

alter table lt_avia_document
drop key aviadocument_commissiondiscount_currency_fk;

alter table lt_avia_document
add KEY `commissiondiscount_currency` ( `commissiondiscount_currency` );

aLTER TABLE `lt_avia_document` ADD CONSTRAINT `aviadocument_commissiondiscount_currency_fk`
	FOREIGN KEY ( `commissiondiscount_currency` ) REFERENCES `lt_currency` ( `id` );
...
update lt_sequence
set name  = 'CashOrderPayment', current = 0
where id = 3;
...
update lt_sequence s1
  inner join (select max(current) c from lt_sequence where id in (1,3)) s2
set s1.current  = s2.c
where s1.id = 5;
