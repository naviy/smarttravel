...
namespace Luxena
{
	...
	public static class LinqExtensions
	{
		...
		[DebuggerStepThrough]
		public static void Register<T>(this IList<T> list, T item)
		{
			if (list == null) return;
		...
		[DebuggerStepThrough]
		public static void Register<T>(this IList<T> list, T? item) where T: struct 
		{
			if (list == null) return;

			if (item.HasValue && !list.Contains(item.Value))
				list.Add(item.Value);
		}

		[DebuggerStepThrough]
		public static void RegisterRange<T>(this IList<T> list, IEnumerable<T> items)
		{
			if (list == null || items == null) return;
		...
		public static IEnumerable<T> Limit<T>(this IEnumerable<T> query, int skipCount, int takeCount)
		{
			...
			if (queryable != null)
				return queryable.Limit(skipCount, takeCount);
