...
#if Local
			TravelUri = "http://v15.bsv.travel.smartikum.com";
			//TravelUri = "http://travel15";
			eOrderNumber.Text = "O.16-02252";
#endif

#if AtlasTour
			TravelUri = "http://v15.atlastour.travel.smartikum.com";
			//TravelUri = "http://travel15";
...
//#endif
				...
				foreach (var a in orderItems)
				{
					sb.AppendFormat("#{0} {1}   {2:n2}   {3}\r\n",
						...
						"A (единый налог)"
#else
							$"А (НДС 20% = {a.Vat:n2} UAH)"
#endif
						: a.Taxtype == 2 ? "Б" : a.Taxtype == 5 ? "Д (Без НДС)" : a.Taxtype.ToString()
					);
				}
				...
				_printOrder = () =>
				{
					...
					try
					{
						var payAmount1 = eTotal2.Text.As().Decimal;
						var payAmount2 = eTotal3.Text.As().Decimal;

						var payType1 = ePayType2.SelectedIndex;
						var payType2 = ePayType3.SelectedIndex;
						...
						if (Math.Abs(payAmount1) >= 0.01m)
						{
							success = Do("Total", () => Driver.printRecTotal((double) payAmount1, payType1), useThrow: true);
							if (!success) return;
						}

						if (Math.Abs(payAmount2) >= 0.01m)
						{
							success = Do("Total", () => Driver.printRecTotal((double) payAmount2, payType2), useThrow: true);
							if (!success) return;
						}
						...
						db.AddToOrderChecks(new OrderCheck
						{
							...
							PayAmount = payAmount1 + payAmount2,
							PaymentType = (CheckPaymentType)payType1,
