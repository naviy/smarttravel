...
namespace Luxena.Domain.Web
{
	...
	public abstract class ReadOnlyEntityODataController<TDomain, TEntity, TKey>
		: DomainODataController<TDomain>
		...
		where TEntity : Domain<TDomain>.Entity<TKey>
	{
		...
		public IQueryable<TEntity> Get(ODataQueryOptions<TEntity> options)
		{
			...
			var id = options.GetIdFilterValue();

			return id.Yes() ? Set : Query;
		}
