		...
		public void Append(MemberInfo sourceMember, IList<Attribute> attrs, DefaultLocalizationTypesSource lng, bool usePropertyPattern = true)
		{
			...
			if (Default.No())
				sourceMember.ResultType().Do(ptype =>
				{
					...
					if (typeName != memberName && "_" + typeName != memberName && typeName != memberName + "Reference") return;

					ptype.Localization(lng).Do(loc => loc.AppendTo(this, lng));

					if (Default.No())
					{
						var pattern = attrs.OfType<MemberPatternAttribute>().One();
						pattern?.Pattern?.Localization(lng).Do(loc => loc.AppendTo(this, lng));
					}
				});
		}
