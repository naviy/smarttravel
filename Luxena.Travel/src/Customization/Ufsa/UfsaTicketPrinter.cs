...
{
	public class UfsaTicketPrinter : ITicketPrinter
	{
		...
		private void AddFrame()
		{
			...
			if (ownerParty != null)
			{
				...
				contentByte.ShowTextAligned(PdfContentByte.ALIGN_CENTER, ownerParty.Email1, 112, yPos + 3, 0);
				contentByte.ShowTextAligned(PdfContentByte.ALIGN_CENTER, ownerParty.Phone1, 204, yPos + 3, 0);
			}
			...
			contentByte.SetFontAndSize(_baseFont, 10);
			contentByte.ShowTextAligned(PdfContentByte.ALIGN_CENTER, Email, 388, yPos + 3, 0);
			contentByte.ShowTextAligned(PdfContentByte.ALIGN_CENTER, Phone, 480, yPos + 3, 0);
		...
		private void AddTicketDataTable()
		{
			...
			cell = new PdfPCell(new Phrase(_currentTicket.Number.Yes() ? string.Format("{0} {1}", _currentTicket.AirlinePrefixCode, _currentTicket.Number) : string.Empty, fontBold))
			{
				HorizontalAlignment = Element.ALIGN_CENTER,
				BorderWidth = 1,
				VerticalAlignment = Element.ALIGN_MIDDLE,
		...
		private void AddCalculation()
		{
			...
			cell = new PdfPCell(new Phrase(_currentTicket.EqualFare != null ? _currentTicket.EqualFare.MoneyString : null, fontBold))
			{
				BorderWidth = 1f,
				HorizontalAlignment = Element.ALIGN_CENTER,
				VerticalAlignment = Element.ALIGN_MIDDLE,
			...
			cell = new PdfPCell(new Phrase(_currentTicket.GrandTotal != null ? _currentTicket.GrandTotal.MoneyString : null, fontBold))
			{
				BorderWidth = 1f,
				HorizontalAlignment = Element.ALIGN_CENTER,
				VerticalAlignment = Element.ALIGN_MIDDLE,
		...
		private void AddPenalties()
		{
			...
			{
				new Chunk(Resources.FareReturnPolicy1_ua + separator, new Font(_baseFontBold, 6, Font.UNDERLINE, _darkBlueColor)),
			...
			{
				new Chunk(Resources.FareReturnPolicy2_ua + separator, new Font(_baseFontBold, 6, Font.UNDERLINE, _darkBlueColor)),
				new Chunk(Resources.FareReturnPolicy2_en, new Font(_baseFont, 6, Font.UNDERLINE, _orangeColor))
			});
			...
			switch (operation.Status)
			{
				...
				case PenalizeOperationStatus.Chargeable:
					...
					if (!string.IsNullOrEmpty(operation.Description))
						text = string.Format("{0} {1}", text, operation.Description);
		...
		private static string GetDepartureDateText(FlightSegment segment)
		{
			...
			{
				if (segment.Carrier != null && !string.IsNullOrEmpty(segment.Carrier.AirlineIataCode))
		...
		private void AddSection(string nameUa, string nameEng, bool isLastSection, params IElement[] items)
		{
			...
			/*if (position - _document.Bottom - table.TotalHeight - table.SpacingBefore <= 0)
				_newPageNeed = true;
			else if (isLastSection && _currentTicket == _tickets[_tickets.Count - 1])
				_insertPageHeader = false;
*/
			if (position - _document.Bottom - table.TotalHeight - table.SpacingBefore >= 0 && isLastSection)
			{
				_newPageInsertionMode = _currentTicket == _tickets[_tickets.Count - 1] ?
