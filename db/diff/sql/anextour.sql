...
CREATE TABLE lt_flight_segment (
    ...
    issidetrip boolean DEFAULT false NOT NULL,
...
CREATE VIEW olap_airport_dim AS
    SELECT a.id, a.code, a.name, a.settlement, c.name AS country FROM (lt_airport a LEFT JOIN lt_country c ON (((c.id)::text = (a.country)::text))) ORDER BY a.code;
...
CREATE VIEW olap_customer_dim AS
    SELECT lt_party.id, lt_party.name FROM lt_party WHERE ((lt_party.id)::text IN (SELECT DISTINCT lt_avia_document.customer FROM lt_avia_document)) ORDER BY lt_party.name;
...
CREATE VIEW olap_document AS
    SELECT lt_avia_document.id, lt_avia_document.issuedate, lt_avia_document.type, lt_avia_document.paymenttype, lt_avia_document.airline, lt_avia_document.seller, lt_avia_document.customer, lt_avia_document.ticketeroffice, CASE WHEN (lt_avia_document.type = 0) THEN lt_avia_document.id ELSE NULL::character varying END AS direction, COALESCE(lt_avia_document.equalfare_amount, (0)::numeric) AS equalfare, COALESCE(lt_avia_document.total_amount, (0)::numeric) AS total, COALESCE(lt_avia_document.commission_amount, (0)::numeric) AS commission, COALESCE(lt_avia_document.servicefee_amount, (0)::numeric) AS servicefee, COALESCE(lt_avia_document.discount_amount, (0)::numeric) AS discount, COALESCE(lt_avia_document.grandtotal_amount, (0)::numeric) AS grandtotal, lt_avia_document.tourcode, (COALESCE(lt_avia_document.total_amount, (0)::numeric) - COALESCE(lt_avia_document.commission_amount, (0)::numeric)) AS totaltotransfer, ((COALESCE(lt_avia_document.commission_amount, (0)::numeric) + COALESCE(lt_avia_document.grandtotal_amount, (0)::numeric)) - COALESCE(lt_avia_document.total_amount, (0)::numeric)) AS profit, lt_avia_document.owner FROM lt_avia_document WHERE (((lt_avia_document.type <> 1) AND (NOT lt_avia_document.requiresprocessing)) AND (NOT lt_avia_document.isvoid)) UNION ALL SELECT d.id, d.issuedate, d.type, d.paymenttype, d.airline, d.seller, d.customer, d.ticketeroffice, t.id AS direction, (- COALESCE(d.equalfare_amount, (0)::numeric)) AS equalfare, (- COALESCE(d.total_amount, (0)::numeric)) AS total, (- COALESCE(d.commission_amount, (0)::numeric)) AS commission, (- COALESCE(d.servicefee_amount, (0)::numeric)) AS servicefee, (- COALESCE(d.discount_amount, (0)::numeric)) AS discount, (- COALESCE(d.grandtotal_amount, (0)::numeric)) AS grandtotal, d.tourcode, (COALESCE(d.commission_amount, (0)::numeric) - COALESCE(d.total_amount, (0)::numeric)) AS totaltotransfer, (((((- COALESCE(d.commission_amount, (0)::numeric)) - COALESCE(d.servicefee_amount, (0)::numeric)) - COALESCE(d.handling_amount, (0)::numeric)) + COALESCE(d.discount_amount, (0)::numeric)) + COALESCE(r.cancelcommission_amount, (0)::numeric)) AS profit, d.owner FROM ((lt_avia_document d JOIN lt_avia_refund r ON (((r.id)::text = (d.id)::text))) LEFT JOIN lt_avia_ticket t ON (((t.id)::text = (r.refundeddocument)::text))) WHERE ((NOT d.requiresprocessing) AND (NOT d.isvoid));
...
CREATE VIEW olap_issuedate_dim AS
    SELECT DISTINCT (date_part('year'::text, lt_avia_document.issuedate))::integer AS year, to_char((lt_avia_document.issuedate)::timestamp with time zone, 'MM Month'::text) AS month, to_char((lt_avia_document.issuedate)::timestamp with time zone, 'DD'::text) AS day, lt_avia_document.issuedate FROM lt_avia_document ORDER BY lt_avia_document.issuedate;
...
CREATE VIEW olap_ticketeroffice_dim AS
    SELECT DISTINCT lt_avia_document.ticketeroffice FROM lt_avia_document ORDER BY lt_avia_document.ticketeroffice;
