...
CREATE VIEW olap_gds_dim AS
    ((SELECT 0 AS type, 'Unknown'::text AS name UNION ALL SELECT 1 AS type, 'Amadeus'::text AS name) UNION ALL SELECT 2 AS type, 'Galileo'::text AS name) UNION ALL SELECT 3 AS type, 'Sirena'::text AS name;
...
--
-- Name: olap_passenger_dim; Type: VIEW; Schema: travel; Owner: travel
--

CREATE VIEW olap_passenger_dim AS
    SELECT DISTINCT btrim(upper((lt_avia_document.passengername)::text)) AS passengername FROM lt_avia_document;


ALTER TABLE travel.olap_passenger_dim OWNER TO travel;
