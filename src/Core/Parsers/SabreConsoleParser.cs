...
namespace Luxena.Travel.Parsers
{
	...
	public class SabreConsoleParser
	{
		...
		private SabreConsoleParser(string content, Currency defaultCurrency)
		{
			Content = content;
		...
		public static IEnumerable<AviaDocument> Parse(string content, Currency defaultCurrency)
		{
			...
			//return new SabreConsoleParser(content, defaultCurrency).ParseTickets();
		}
		...
		static readonly Regex _reHeader = new Regex(
			@"\s*(?<PnrCode>\w+)\s*(?<passangersAndSegments>.+?)PHONES.+?REMARKS(?<remarks>.*)",
			RegexOptions.Singleline | RegexOptions.Compiled
		);

		static readonly Regex _rePassangers = new Regex(
			@"\s*(?<passangerNo1>\d\.)(?:\w\/)?(?<passangerNo2>\d)(?<passanger>.+?)(?=[\d*])",
			RegexOptions.Singleline | RegexOptions.Compiled
		);
		...
		static readonly Regex _reMaskList = new Regex(
			@"^\s+(?<passangerNo>\d\.\d)\s.+?\s(?<maskNo>\d+)\s",
			RegexOptions.Multiline | RegexOptions.Compiled
		);
